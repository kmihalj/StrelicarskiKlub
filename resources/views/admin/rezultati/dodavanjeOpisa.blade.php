...
<div class="row">
    ...
    <div class="col-12 mb-2">
        <form id="dodatniPodaci" action="{{ route('admin.rezultati.dodatniPodaciRezultat') }}" method="POST">
            @csrf
            <input type="hidden" id="turnir_id" name="turnir_id" value={{$turnir->id}}>
            <label for="opis_turnira">Opis turnira (tekst ispod tablice na prikazu rezultata):</label>
            <textarea class="form-control ck-content" name="opis_turnira" id="opis_turnira" aria-label="opis_turnira">{{ $turnir->opis }}</textarea>
        </form>
    </div>
    <div class="col-lg-12 col-md-12 col-12 mb-2 text-end">
        <button type="submit" form="dodatniPodaci" class="btn btn-danger">Spremi</button>
    </div>
</div>
